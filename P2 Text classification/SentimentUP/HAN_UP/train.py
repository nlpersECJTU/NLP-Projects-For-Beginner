...
def eval(model, data):
    ...
    with torch.no_grad():
        for batch in utils.batch_iter(data, conf.batch_size, True):
            u, p, x, y, sen_lens, doc_lens = batch
            u = torch.LongTensor(u).to(conf.device)
            p = torch.LongTensor(p).to(conf.device)
            ...
            predicts = model(u, p, x, sen_lens, doc_lens)
...
def train(conf, train_data, dev_data, optimizer, criterion):
    ...
    logg.info("Best dev acc: %.3f, Best dev rmse: %.3f" % (best_dev_acc, best_dev_rmse))
    return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--user_dim', type=int)
    parser.add_argument('--prod_dim', type=int)
    ...
    # create user and product dictionary
    user_dict, prod_dict = utils.load_up_dict(conf)
    conf.user_num = len(user_dict)
    conf.prod_num = len(prod_dict)
    ...
    train_data = utils.load_data(conf, vocab, user_dict, prod_dict, 'train')
    logg.info('Loading dev data')
    dev_data = utils.load_data(conf, vocab, user_dict, prod_dict, 'dev')
    logg.info('Loading test data')
    test_data = utils.load_data(conf, vocab, user_dict, prod_dict, 'test')
    
    # build and train model
    model = HAN_UP(conf, embeddings)
