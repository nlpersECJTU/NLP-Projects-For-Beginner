...
def load_up_dict(conf):
    ...
    for fn in fn_list:
        with open(fn, 'r') as f:
            for line in f:
                u, p, _, _ = [x.strip() for x in line.strip().split('\t\t')]
                user_set.add(u)
                prod_set.add(p)
...
def load_data(conf, vocab, user_dict, prod_dict, dataset = 'train'):
    if dataset == 'train':
        fl = conf.dtrain
    if dataset == 'dev':
        fl = conf.ddev
    ...
    with open(fl, 'r') as f:
        for line in tqdm(f):
            ...
            data_u.append(user_dict[line[0]])
            data_p.append(prod_dict[line[1]])
            data_x.append(line[2].lower())
            data_y.append(int(line[3]) - 1)

    # word -> idx
    # cut long sentence/document, pad short sentence/document
    x, sent_len, doc_len = [], [], []
        ...
        for sen in sentences:
            ...
            for word in sen.strip().split():
                ...
                if word not in vocab:
                    # words Out Of Vocab (OOV) are discarded
                    continue
                t_x[i, j] = vocab[word]
                j += 1
            t_sen_len[i] = j
    ...
    return data_u, data_p, data_x, data_y, sent_len, doc_len


# batch data
def batch_iter(data_set, batch_size, shuffle=True):
    data_u, data_p, data_x, data_y, sen_lens_x, doc_lens_x = data_set
    data_size = data_x.shape[0]
    num_batches = int((data_size - 1) / batch_size) + 1

    if shuffle:
        shuffled_indices = np.random.permutation(np.arange(data_size))
        data_u = data_u[shuffled_indices]
        data_p = data_p[shuffled_indices]
    ...
    for i in range(num_batches):
        ...
        yield (data_u[start_id:end_id], data_p[start_id:end_id], 
               data_x[start_id:end_id], data_y[start_id:end_id],
